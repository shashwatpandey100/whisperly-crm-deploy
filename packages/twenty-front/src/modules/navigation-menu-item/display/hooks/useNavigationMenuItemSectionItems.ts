...
export const useNavigationMenuItemSectionItems = (): NavigationMenuItem[] => {
  ...
  // A folder with no readable children is empty from this member's
  // perspective - drop it too, unless it's being edited (an admin
  // customizing the layout still needs to see and reopen it).
  const foldersWithChildrenIds = new Set(
    workspaceNavigationMenuItemsByFolder.map((folder) => folder.id),
  );
  const visibleFlatItems = isLayoutCustomizationModeEnabled
    ? flatItems
    : flatItems.filter(
        (item) =>
          item.type !== NavigationMenuItemType.FOLDER ||
          foldersWithChildrenIds.has(item.id),
      );
